...
const AppContent: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 flex flex-col lg:flex-row relative">
      ...
      <nav className={`hidden lg:flex flex-col w-60 h-screen border-r border-gray-200 bg-white p-6 ${sidebarOpen ? 'sticky' : 'fixed'} top-0 overflow-y-auto sidebar-scrollbar transition-transform duration-300 z-50 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mb-8 relative group">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <svg className="h-4 w-4 text-gray-400 group-focus-within:text-purple-400 transition" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <input 
            ...
            className="block w-full pl-10 pr-3 py-3 border border-gray-200 rounded-2xl bg-gray-50 leading-5 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-purple-400 sm:text-sm transition"
          />
        </div>
      ...
      {/* Main Content Area */}
      <main className={`flex-1 relative h-screen overflow-y-auto overflow-x-hidden scrollbar-hide transition-all duration-300 bg-white ${sidebarOpen ? 'lg:ml-0' : 'lg:ml-0'}`}>
        {/* Hamburger button when sidebar is closed */}
        {!sidebarOpen && (
          <button
            onClick={() => setSidebarOpen(true)}
            className="hidden lg:flex fixed top-4 left-4 z-50 p-3 rounded-lg bg-white hover:bg-gray-50 border border-gray-200 transition text-gray-600 hover:text-gray-900 shadow-lg"
