...
const BettingOverlay: React.FC<BettingOverlayProps> = ({ event, user, onPlaceBet, onClose }) => {
  ...
  return (
    <div className="absolute inset-0 bg-black/80 flex flex-col justify-end p-6 z-50 animate-in slide-in-from-bottom duration-300">
      <div className="bg-zinc-900 rounded-t-3xl p-6 border-t border-purple-500/30">
        <div className="flex justify-between items-start mb-6">
          <div>
            <h3 className="text-xl font-bold text-white mb-1">{event.question}</h3>
        ...
        <div className="space-y-3 mb-6">
          {event.options.map((option) => (
            ...
            >
              ...
              <span className="text-purple-400 font-mono">x{option.odds}</span>
            </button>
          ))}
        </div>

        <div className="mb-6">
          <label className="block text-sm text-zinc-400 mb-2">Bet Amount (Available: ${user.balance})</label>
          <div className="flex items-center space-x-4">
            <input
              type="number"
              value={amount}
              onChange={(e) => setAmount(Number(e.target.value))}
              className="flex-1 bg-zinc-800 border border-zinc-700 rounded-xl p-3 focus:outline-none focus:border-purple-500 font-bold text-xl"
            />
            <div className="flex space-x-2">
              {[10, 50, 100].map(val => (
                <button 
                  ...
                  className="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 rounded-lg text-sm border border-zinc-700 transition"
                >
                  +{val}
                </button>
              ))}
        ...
        <button
          ...
          className="w-full py-4 bg-purple-600 hover:bg-purple-700 disabled:bg-zinc-800 disabled:text-zinc-600 rounded-xl font-bold text-lg shadow-lg shadow-purple-500/20 transition-all active:scale-95"
        >
